...
#define BALENA_IDAC 0x02
#define BALENA_IDAC_SET_RESOLUTION 0x00
#define BALENA_IDAC_WRITE_STEP 0x01
#define BALENA_IDAC_RESET 0x02
...
byte balenaHardwareConfigGet(byte pin, byte config)
{
  if (config == 0x01)
  {
    byte res = IS_PIN_PWM(pin) | (IS_PIN_IDAC(pin) << 1);
    return res;
  }
  else if (config == 0x00)
  {
    return (byte)port_pin[pin].state;
  }
  else
  {
    Firmata.sendString("Balena Error: Outside range.");
    return BALENA_ERR;
  }
}

byte balenaHardwareConfigSet(byte pin, byte mode)
{
  if ((mode == (MODE_ANALOG_OUT)) || (mode == (MODE_PWM)) || (mode == (MODE_DIGITAL_INPUT)))
  {
    port_pin[pin].state = mode;
    return BALENA_SUCCESS;
  }
  Firmata.sendString("Balena Error: Invalid mode.");
  return BALENA_ERR;
}
...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  case CAPABILITY_QUERY:
    ...
    for (byte pin = 0; pin < TOTAL_PINS; pin++)
    {
      ...
      if (port_pin[pin].adc != MODE_DIGITAL_INPUT)
      {
    ...
    case BALENA_SLEEP:
      if (argc > 5)
      {
        ...
        power_struct.sleep_period = (((argv[5] << 21) & 0x01) |
                                     ...
                                     (argv[3] << 7) |
                                     (argv[2])) *
                                    (uint32_t)DELAY_MULTIPLIER; // in seconds
        if (argv[1] == 0)
      ...
      {
        Firmata.sendString("Balena Error: Missing arguments.");
      }
      break;
    case BALENA_IDAC:
      if (argc == 4)
      {
        if (argv[1] == BALENA_IDAC_SET_RESOLUTION)
        {
        ...
        else if (argv[1] == BALENA_IDAC_WRITE_STEP)
        {
        ...
        {
          Firmata.sendString("Balena Error: Invalid payload.");
        }
      }
      else if (argc == 2)
      {
        if (argv[1] == BALENA_IDAC_RESET)
          resetIDAC();
        else
          Firmata.sendString("Balena Error: Invalid payload.");
      }
